...
pub async fn update(
    ...
    // Cek user exist
    let user_exist = match sqlx::query!("SELECT id FROM users WHERE id = ?", id)
        .fetch_one(&db)
        .await
    {
        Ok(user) => user,
...
pub async fn destroy(
    ...
    // Cek user exist
    let user = match sqlx::query!("SELECT id FROM users WHERE id = ?", id)
        .fetch_one(&db)
        .await
    {
        Ok(user) => user,
    ...
    let result = sqlx::query!("DELETE FROM users WHERE id = ?", user.id)
        .execute(&db)
        .await;
